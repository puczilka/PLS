...
h_tol <- .Machine$double.eps ^ 0.5     # Tolerance for heaviside application in iterative step
# Mathematically chosen to be 0 but can be a value close to 0 (based on machine precision) to cancel out weak signals
...
for (group in 1:k) {
  vec <- M_v[(x.blocks[[group]])]
  tmp <- c(tmp, max(h_tol, (1 - (lambda_u/x.penalties[group])))*vec)
}
...
#     if(y < tol) y <- 0     #### NOTE: y is a scalar. The scalar is zeroed out if it is below the tolerance
#     res <- c(res,vecx*y)
#   # }
#   # return(res)
#   # }
...
for (group in 1:l) {
  vec <- M_u[(y.blocks[[group]])]
  tmp <- c(tmp, max(h_tol, (1 - (lambda_v/y.penalties[group])))*vec)
}
...
for (group in 1:l) {
  vec <- M_u[(y.blocks[[group]])]
  temp <- c(temp, max(h_tol, (1 - (lambda_v/y.penalties[group])))*vec)
}

temp2 = soft.thresholding.group(M_u,ind=ind.block.y,lambda=lambda_v)



# tmp <- NULL
#     vec <- M_v[(x.blocks[[1]])]
#     tmp <- c(tmp, max(0, (1 - (lambda_u/x.penalties[1])))*vec)
#   u_new = tmp
# 
#   # soft.thresholding.group <- function(x,ind,lambda){
#   tab.ind <- c(0,ind.block.x,length(M_v))
