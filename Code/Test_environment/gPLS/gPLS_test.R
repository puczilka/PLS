...
# Define 'Euclidean norm' function
eu_norm = function 
...
# Set tolerances
tol <- 1e-06     # Tolerance for difference between iterative steps
pen_tol <- .Machine$double.eps ^ 0.5     # Tolerance for penalty. This accounts for truncation errors from nonlinear
# projections AND heaviside application on penalty
...
for (group in 1:k) {
  ...
  penalty[group] <- 2*sqrt(as.vector(t(vec) %*% vec))
  x.penalties <- c(x.penalties, 2*sqrt(as.vector(t(vec) %*% vec))/sqrt(p_k[group]))
}
...
u_new = tmp / sqrt(as.vector(t(tmp) %*% tmp))
...
#   # for (group in 1:k) {
#     vec <- M_v[(x.blocks[[1]])]
#     tmp <- c(tmp, pmax(0, (1 - (lambda_u/x.penalties[1]))*vec))
#   # }
#   u_new = tmp
# 
#
#
#   tab.ind <- c(0,ind.block.x,length(M_v))
#   h_tol <- .Machine$double.eps ^ 0.5
#   res <- NULL
#   # for (i in 1:(length(ind)+1)){
#     ji <- tab.ind[1+1]-tab.ind[1]
#     vecx <- M_v[((tab.ind[1]+1):tab.ind[1+1])]
#     y <- 1-(lambda_u/2)*sqrt(ji)/normv(vecx)
#     if(y < h_tol) y <- 0     #### NOTE: y is a scalar. The scalar is zeroed out if it is below the tolerance
#     res <- c(res,vecx*y)
#   # }
#   u_new2 = res
# 
# 
# 
u_new2 = soft.thresholding.group(M_v,ind=ind.block.x,lambda=lambda_u)
u_new2 = u_new2 / sqrt(as.vector(t(u_new2) %*% u_new2))
# 
# 
#
# ### Test penalties (sparsity)
vecZV <- M %*% matrix(v_old, ncol=1)
...
for (group in 1:l) {
  ...
  penalty[group] <- 2*sqrt(as.vector(t(vec) %*% vec))
  y.penalties <- c(y.penalties, 2*sqrt(as.vector(t(vec) %*% vec))/sqrt(q_l[group]))
}
...
v_new = tmp / sqrt(as.vector(t(tmp) %*% tmp))
...
# # for (group in 1:l) {
# vec <- M_u[(y.blocks[[1]])]
# temp <- c(temp, pmax(0, (1 - (lambda_v/y.penalties[1]))*vec))
# # }
# 
# 
# 
# vecZU <- t(M) %*% matrix(u_new, ncol=1)
# tab.ind <- c(0,ind.block.y,length(vecZU))      # Add 0 and p into ind.block.x
# res <- NULL
# pen <- c()
# 
# # Iterate over each group
...
# 
#   # Define the size of each group, q_l
#   ji <- tab.ind[i+1]-tab.ind[i]
# 
#   # Return the entries corresponding to each group of the projection of the loadings
#   vecy <- vecZU[((tab.ind[i]+1):tab.ind[i+1])]
#   pen[i] <- 2*normv(vecy)
#   res <- c(res,2*normv(vecy)/sqrt(ji))     # Create vector of 'multipliers' for group lasso computation
# }
# y.pen = res



v_new2 = soft.thresholding.group(M_u,ind=ind.block.y,lambda=lambda_v)
v_new2 = v_new2 / sqrt(as.vector(t(v_new2) %*% v_new2))
...
# 
# 
# sgPLS package soft thresholding
# 
#   tab.ind <- c(0,ind.block.y,length(M_u))
#   h_tol <- .Machine$double.eps ^ 0.5
#   res <- NULL
#   # for (i in 1:(length(ind)+1)){
#     ji <- tab.ind[1+1]-tab.ind[1]
...
# res.load <- step1.group.spls.sparsity(X = X_h, Y_h, ind.block.x = ind.block.x, 
#                                       ind.block.y = ind.block.y, sparsity.x = x_sparsity[1], 
#                                       sparsity.y = y_sparsity[1], epsilon = tol, iter.max = 1)


model.gPLS <- gPLS(X, Y, ncomp = n_comp, mode = "regression", keepX = c(4, 4), 
                   keepY = c(4, 4), ind.block.x = ind.block.x , ind.block.y = ind.block.y, max.iter = 1)
