...
for (group in 1:k) {
  vec <- M_v[(x.blocks[[group]])]
  tmp <- c(tmp, max(0, (1 - (lambda_u/x.penalties[group])))*vec)
}
u_new = tmp / normv(tmp)
...
for (group in 1:l) {
  vec <- M_u[(y.blocks[[group]])]
  tmp <- c(tmp, max(0, (1 - (lambda_v/y.penalties[group])))*vec)
}
v_new = tmp / normv(tmp)
...
model.gPLS <- gPLS(X, Y, ncomp = n_comp, mode = "regression", keepX = c(4, 4), 
                   keepY = c(4, 4), ind.block.x = ind.block.x , ind.block.y = ind.block.y, max.iter = 1, scale = FALSE)
