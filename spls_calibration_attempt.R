...
# # ------------------- TEST MSEP ------------------- #
# test <- spls(X_train, Y_train, ncomp = 1, keepX = c(3) , mode = "regression")
# test_pred <- predict(test, X_val)
# test_pred <- test_pred$predict[ , , 1]
# 
...
for (var_no in (1:max_X_vars)) {
  tmp[comp] <- c(var_no)
  model_A <- spls(X_train, Y_train, ncomp = comp, keepX = tmp, mode = "regression")
  pred <- predict(model_A, X_val)
